...
from commands import note as note_cmd
...
def create_note(req: NoteCreate):
    ctx = load_ctx()
    result = note_cmd.handle(ctx, f"/note {req.content}")
    if not result.ok:
...
def delete_note(note_id: int):
    ctx = load_ctx()
    result = note_cmd.handle(ctx, f"/note delete {note_id}")
    if not result.ok:
...
def export_to_vault():
    ctx = load_ctx()
    result = note_cmd.handle(ctx, "/note export")
    if not result.ok:
        raise HTTPException(status_code=400, detail=result.message)
